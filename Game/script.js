...
var data2 = {
    ...
    var animation3 = new createjs.Sprite(spriteSheet2);
	animation3.scaleX = 0.25;
	animation3.scaleY = 0.15;
		var data3 = {
        images: ["build.png"],
    ...
    var animation2 = new createjs.Sprite(spriteSheet3);
	animation2.scaleX = 0.25;
	
	animation2.scaleY = 0.25;
var data4 = {
        images: ["fond.png"],
    ...
    var spriteSheet4 = new createjs.SpriteSheet(data4);
    var animation4 = new createjs.Sprite(spriteSheet4);
	animation4.scaleX = 0.5;
	
	animation4.scaleY = 0.3;
	...
	if (joueurs[numm][1] == document.querySelector('h1').innerHTML && hbol) {//&& verifAttaquer == attaquer
       ...
       var mathPosPersoX2 = Math.round(mathPosPersoX+xc);//-0.5
		...
		if (!page.muraille[(mathPosPersoX2)]) {
						...
						}  else if (Number(page.muraille[(mathPosPersoX)][(mathPosPersoY)]) == 0) {
							tiimer2 = false;
			document.getElementById("name").value = document.getElementById("h1").innerHTML;
							document.getElementById("map").value = sss+1;
				...
				tiimer2 = false;
							...
							var herbe = 1;
								for (var zr = 0; zr < Qinv.length; zr ++) {
								for (var zr2 = 0; zr2 < Qinv[zr].length; zr2 ++) {
									if (Qinv[zr][zr2].split("*")[1].split("%")[0] == "Cuivre") {
										Qinv[zr][zr2] = Qinv[zr][zr2].split("%")[0] + "%" + (Number(Qinv[zr][zr2].split("%")[1])+1).toString();
										herbe = 2;
										break;
									}
								...
								for (var zr2 = 0; zr2 < Qinv[zr].length; zr2 ++) {
									if (Qinv[zr][zr2].split("*")[1].split("%")[0] == "VIDE") {
										Qinv[zr][zr2] = "LoadImage~build.png*Cuivre%" + (Number(Qinv[zr][zr2].split("%")[1])+1).toString();
										caillou = 2;
										break;
									}
								}
						...
						if (Number(page.muraille[(mathPosPersoX)][(mathPosPersoY)]) == 0 && hbolwin && joueurs[numm][1] == document.querySelector('h1').innerHTML) {
							hbolwin = false;
							document.getElementById("name").value = document.getElementById("h1").innerHTML;
							document.getElementById("map").value = Number(document.getElementById("map").value)+1;
							document.getElementById("windo").click();
...
joueurs.push([ ["", "", "","","","",""] ]);
		...
		personn.x = Math.trunc(Math.random()*100+(Math.round(Math.random()*4)*1400))+50;
		personn.y = Math.trunc(Math.random()*100+(Math.round(Math.random()*4)*1200))+50;
		} else {
			personn.x = 100;
			personn.y = 100;
		}
		...
		mesha = personn;
			...
			page.getChildByName(inittss).addEventListener("mousedown", function(evt) {
				
			console.log(inittss + "=" + page.muraille[Math.trunc((clientX-page.x)/32)][Math.trunc((clientY-page.y)/32)]);
...
var containerMapping = new createjs.Container();
		...
		for (var fi=0; fi <= 151; fi++) {
			for (var ji=0; ji <= 144; ji++) {
				...
				*/
				...
				if (((ji >= 1 && ji <= 4) || (ji <= 143 && ji >= 139)) || ((fi <= 150 && fi >= 146) || (fi >= 1 && fi <= 4))) {
				var mapping;
	...
	mapping.scaleY = saac;
				...
				if (gototo == -1 || gototo == 0) {
					mapping.gotoAndStop(Math.abs(gototo));
					mapping.name = "map" + fi.toString() + ji.toString();
				containerMapping.addChild(mapping);
